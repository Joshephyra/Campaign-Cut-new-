...
describe('per-project element timing', () => {
  ...
  it('starts from the template defaults, all enabled', () => {
    expect(db.getProjectElements(projectId)).toEqual([
      { id: open, slug: 'open', zIndex: 0, startFrame: 0, endFrame: 150, enabled: true },
      { id: card, slug: 'end-card', zIndex: 1, startFrame: 150, endFrame: 300, enabled: true },
    ]);
  });
  ...
  it('stores an in/out move and a toggle per project without touching the template', () => {
    ...
    expect(db.getProjectElements(projectId)).toEqual([
      { id: open, slug: 'open', zIndex: 0, startFrame: 0, endFrame: 150, enabled: false },
      { id: card, slug: 'end-card', zIndex: 1, startFrame: 120, endFrame: 270, enabled: true },
    ]);
