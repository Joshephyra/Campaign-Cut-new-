...
describe('per-project element timing', () => {
  ...
  it('starts from the template defaults, all enabled', () => {
    expect(db.getProjectElements(projectId)).toEqual([
      { id: open, elementId: open, slug: 'open', name: 'open', type: 'overlay', templateSlug: 't', added: false, zIndex: 0, startFrame: 0, endFrame: 150, enabled: true },
      { id: card, elementId: card, slug: 'end-card', name: 'end-card', type: 'overlay', templateSlug: 't', added: false, zIndex: 1, startFrame: 150, endFrame: 300, enabled: true },
    ]);
  });
  ...
  it('stores an in/out move and a toggle per project without touching the template', () => {
    ...
    expect(db.getProjectElements(projectId)).toEqual([
      { id: open, elementId: open, slug: 'open', name: 'open', type: 'overlay', templateSlug: 't', added: false, zIndex: 0, startFrame: 0, endFrame: 150, enabled: false },
      { id: card, elementId: card, slug: 'end-card', name: 'end-card', type: 'overlay', templateSlug: 't', added: false, zIndex: 1, startFrame: 120, endFrame: 270, enabled: true },
    ]);
