...
const detailWith = (endCardEnd: number) => ({
  project: { id: 7, name: 'Demo', templateId: 1, templateSlug: 'demo', templateName: 'Demo', lengthS: endCardEnd / 30 }, // M52: the spot is as long as its content, so only the disclaimer decides
  ...
  elements: [
    ...
    { id: 4, slug: 'end-card', name: 'End card', type: 'end-card', templateSlug: 'demo', added: false, zIndex: 0, startFrame: 225, endFrame: endCardEnd, enabled: true, lottieUrl: '/templates/demo/elements/end-card/template.json', schema: [disclaimer] },
  ],
  transitions: [],
  meta: { fonts: [], fontFiles: [] },
  values: [{ elementId: 4, key: 'disclaimer', value: 'Paid for by Example Committee' }],
});
const detail = detailWith(300);

function mockApi(d: ReturnType<typeof detailWith> = detail) {
  vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
    const url = String(input);
    if (url === '/api/projects/7' && !init?.method) return new Response(JSON.stringify(d), { status: 200 });
...
describe('Editor disclaimer check (M35)', () => {
  it('blocks Export under four seconds with the reason, and is free on a template whose end card is long enough', async () => {
    mockApi();
    ...
    cleanup();
    vi.restoreAllMocks();
    // M52: timing is the designer's. The same spot on a template whose end card runs 5 s is free to export.
    mockApi(detailWith(375));
    render(<Editor projectId={7} onBack={() => {}} />);
    await waitFor(() => expect(screen.getByTestId('readiness')).toBeTruthy());
